#!/bin/bash

# The raw transaction
TX=""

# Decode transaction
DECODED=$(bitcoin-cli -regtest decoderawtransaction "$TX" true)

# Extract public keys from each input's witness data
# Last item in each witness array is the public key (minus the 21 prefix)
PUBKEYS=$(echo "$DECODED" | jq -r '.vin[].txinwitness[-1]' | sed 's/^21//')

# Format public keys into JSON array
PUBKEY_JSON='['
for pubkey in $PUBKEYS; do
    PUBKEY_JSON="$PUBKEY_JSON\"$pubkey\","
done
PUBKEY_JSON=${PUBKEY_JSON%,}  # Remove trailing comma
PUBKEY_JSON="$PUBKEY_JSON]"

# Create multisig address
P2SH_INFO=$(bitcoin-cli -regtest createmultisig 1 "$PUBKEY_JSON")

# Output only the address
echo "$P2SH_INFO" | jq -r '.address'