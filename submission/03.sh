#!/bin/bash

# The raw transaction
TX=""

# Decode the transaction and extract public keys from witness data
echo "Decoding transaction and extracting public keys..."
DECODED=$(bitcoin-cli -regtest decoderawtransaction $TX true)

# Extract public keys from the witness data
# Each input's witness data contains a signature and a public key
# We want the last element of each witness array (the public key)
PUBKEY1=$(echo $DECODED | jq -r '.vin[0].txinwitness[-1]' | cut -c 3-)  # Remove the '21' prefix
PUBKEY2=$(echo $DECODED | jq -r '.vin[1].txinwitness[-1]' | cut -c 3-)
PUBKEY3=$(echo $DECODED | jq -r '.vin[2].txinwitness[-1]' | cut -c 3-)
PUBKEY4=$(echo $DECODED | jq -r '.vin[3].txinwitness[-1]' | cut -c 3-)

# Print extracted public keys
echo "Extracted Public Keys:"
echo "PUBKEY1: $PUBKEY1"
echo "PUBKEY2: $PUBKEY2"
echo "PUBKEY3: $PUBKEY3"
echo "PUBKEY4: $PUBKEY4"

# Create 1-of-4 multisig address
echo -e "\nCreating 1-of-4 multisig address..."
MULTISIG_INFO=$(bitcoin-cli -regtest createmultisig 1 "[\"$PUBKEY1\",\"$PUBKEY2\",\"$PUBKEY3\",\"$PUBKEY4\"]")

# Print the result
echo "$MULTISIG_INFO"