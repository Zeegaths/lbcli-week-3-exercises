#!/bin/bash

# The raw transaction to decode
TX=""

# Get transaction details
DECODED=$(bitcoin-cli -regtest -named decoderawtransaction hexstring=$TX)

# Extract TXID and VOUT from decoded transaction
TXID=$(echo "$DECODED" | jq -r '.txid')
VOUT=0
RECIPIENT="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
AMOUNT=0.20000000

# Create the PSBT following the docs example
rawtx=$(bitcoin-cli -regtest -named createrawtransaction \
  inputs='''[ { "txid": "'$TXID'", "vout": '$VOUT', "sequence": 4294967293 } ]''' \
  outputs='''{ "'$RECIPIENT'": '$AMOUNT' }''')

# Convert to PSBT
psbt=$(bitcoin-cli -regtest -named converttopsbt hexstring=$rawtx)

# Output only the PSBT
echo $psbt