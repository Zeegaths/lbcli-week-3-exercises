# Create a partially signed transaction from the details below

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""
#!/bin/bash

#!/bin/bash

# Set the variables from the provided information
txid="77efbd235f00d1d92c77e3e4b2a5f6d9954547d0865f8d695eecbbde8e92b0c8"
vout=37

# Create a PSBT using the specific inputs and outputs
psbt=$(bitcoin-cli -regtest walletcreatefundedpsbt \
  '[{"txid":"'$txid'", "vout":'$vout'}]' \
  '[{"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP": 0.20}]')

# Extract just the PSBT string from the JSON response
psbt=$(echo $psbt | jq -r '.psbt')

# Output only the PSBT string
echo $psbt